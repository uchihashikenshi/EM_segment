...
mem_cGAN_home = os.getcwd() # projects/memCNNから走らせる予定
...
class Preprocessing(object):
    ...
    @staticmethod
    def load_tif_images(data_dir):
        os.chdir("data/%s" % data_dir)
        files = glob.glob('*.tif')
        os.chdir("%s" % mem_cGAN_home)
        return files
    ...
    def patch_extract(self, data_dir, label_data_dir, prefix = "", image_size = 256, crop_size = 33, stride = 5):
        ...
        """
        filelist = self.load_tif_images(data_dir)
        labellist = self.load_tif_images(label_data_dir)

        center = (crop_size - 1) / 2

        # training dastaset作成
        if os.path.exists("%s/data/training_dataset/%straining_dataset" % (mem_cGAN_home, prefix)) != True:
            os.mkdir("%s/data/training_dataset/%straining_dataset" % (mem_cGAN_home, prefix)) # trainingデータ置き場用意
        else:
            # 既にdatasetが存在すればエラーを返す
            print "%s/data/training_dataset/%straining_dataset already exist." % (mem_cGAN_home, prefix)
            return
        # test dataset作成
        if os.path.exists("%s/data/test_dataset/%stest_dataset" % (mem_cGAN_home, prefix)) != True:
            os.mkdir("%s/data/test_dataset/%stest_dataset" % (mem_cGAN_home, prefix)) # testデータ置き場用意
        else:
            # 既にdatasetが存在すればエラーを返す
            print "%s/data/test_dataset/%stest_dataset already exist." % (mem_cGAN_home, prefix)
            return
        # reconstruction test dataset作成
        if os.path.exists("%s/data/test_dataset/reconstruction_%stest_dataset" % (mem_cGAN_home, prefix)) != True:
            os.mkdir("%s/data/test_dataset/reconstruction_%stest_dataset" % (mem_cGAN_home, prefix)) # testデータ置き場用意
        else:
            # 既にdatasetが存在すればエラーを返す
            print "%s/data/test_dataset/reconstruction_%stest_dataset already exist." % (mem_cGAN_home, prefix)
            return

        # trainig, testのデータベース作成用txt(名前は全てtraining.txt, test.txt)
        training_f = open("%s/data/training_dataset/%straining_dataset/training.txt" % (mem_cGAN_home, prefix), 'w')
        test_f = open("%s/data/test_dataset/%stest_dataset/test.txt" % (mem_cGAN_home, prefix), 'w')
        # reconstruction testのデータベース作成用txt(名前は全てtraining.txt, test.txt)
        reconstruction_test_f = open("%s/data/test_dataset/reconstruction_%stest_dataset/test.txt" % (mem_cGAN_home, prefix), 'w')
        ...
        for file, label in zip(filelist, labellist):
            ...
            for h in xrange(int((image_size - crop_size) / stride)):
                for w in xrange(int((image_size - crop_size) / stride)):
                    ...
                    cropped_image = Image.open("%s/data/%s/%s" % (mem_cGAN_home, data_dir, file)).crop(patch_range)
                    cropped_label = Image.open("%s/data/%s/%s" % (mem_cGAN_home, label_data_dir, label)).crop(patch_range)
                    ...
                    if (ans_0_number >= ans_1_number and ans == 1) or (ans_0_number <= ans_1_number and ans == 0):
                        ...
                        # 保存部分
                        if file_index <= 80:
                            cropped_image.save("%s/data/training_dataset/%straining_dataset/%straining_image_%03d%03d%03d.tif" % (mem_cGAN_home, prefix, prefix, file_index, h, w))
                            training_f.write("%straining_image_%03d%03d%03d.tif %s\n" % (prefix, file_index, h, w, ans))
                        else:
                            cropped_image.save("%s/data/test_dataset/%stest_dataset/%stest_image_%03d%03d%03d.tif" % (mem_cGAN_home, prefix, prefix, file_index, h, w))
            ...
            if file_index == 90:
                for h in xrange(int(image_size - crop_size)):
                    for w in xrange(int(image_size - crop_size)):
                        ...
                        cropped_label = Image.open("%s/data/%s/%s" % (mem_cGAN_home, label_data_dir, label)).crop(patch_range)
                        ans = int(np.array(list((cropped_label.getdata()))).reshape((crop_size, crop_size))[center][center] / 255)
                        cropped_image.save("%s/data/test_dataset/reconstruction_%stest_dataset/reconstruction_%stest_image_%03d%03d%03d.tif" % (mem_cGAN_home, prefix, prefix, file_index, h, w))
